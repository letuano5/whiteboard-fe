...
transient (interaction store only). Resize supports crossing the fixed anchor with normalized
positive bounds and logical handle flipping. Angle = 0 throughout P1A.
...
| IV | ShapeUtil Strategy — no type branching in core; new shape = new ShapeUtil only | ✅ | Bounds logic is type-agnostic; point geometry is transformed only when `props.points` exists |
| V | Sync Data Not Renderer — only `Element[]` crosses boundaries | ✅ (N/A) | No sync in this feature; drag is transient |
| VI | Single Mutation Pipeline — `createElement`/`patchElement`/`deleteElements`/`updateElements` only | ✅ | Move/resize, including point geometry flips, commit once through `patchElement`; Delete → `deleteElements([id])` |
| VII | Committed vs Transient State — `elements.store` and `interaction.store` stay separate | ✅ | `dragStart`, `resizeHandle`, `resizeSession`, `draggingId` live in interaction store; committed element updated only on commit |
...
├── store/
│   └── interaction.store.ts         [MODIFY] add drag fields and fixed resize session
└── canvas/tools/__tests__/
    └── select-tool.test.ts          [MODIFY] add move/resize/delete/flip tests
```
...
| `resizeHandle` | `ResizeHandleId \| null` | Current logical handle (`null` = body drag = move) |
| `resizeSession` | `ResizeSession \| null` | Original bounds, original handle, and fixed anchor for the current resize |
...
**Decision 5 — Minimum size and resize-with-flip**

Keep the minimum-size rule inside resize geometry, not `patchElement`. Capture a fixed resize session at pointer-down. Compute positive bounds from `min/max(anchor, pointer)` and flip the logical handle when the pointer crosses the anchor. This keeps `patchElement` thin while allowing design-tool-style resize crossing without negative dimensions.
