...
**Independent Test**: Select a shape, drag its `se` corner handle — width and height must grow/shrink. Drag `nw` — both position and size must change. No dimension should go below 1 world unit.
...
- What happens when a resize drag reduces a dimension to zero or negative? → Clamp to 1 world unit minimum.
...
- **FR-006**: Resize MUST clamp `width` and `height` to a minimum of 1 world unit; the shape MUST NOT invert.
...
- **InteractionState** (`selectedIds`, `draggingId`, `dragStart`, `resizeHandle`): Transient state tracking the in-progress drag operation.
